# Nudge Tiny Budget Board

Date: 2026-09-17

Tiny app concept: a retro terminal **tiny budget board**.

Core touches:

- progress rings
- keyboard shortcuts
- localStorage persistence

Palette:

- Background: `#1b1b3a`
- Accent: `#693668`
- Text: `#a74482`

Starter file:

```html
<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Nudge Tiny Budget Board</title>
  <style>
    body {
      margin: 0;
      min-height: 100vh;
      display: grid;
      place-items: center;
      background: #1b1b3a;
      color: #a74482;
      font-family: system-ui, sans-serif;
    }
    main {
      width: min(34rem, calc(100% - 2rem));
      border: 1px solid #693668;
      border-radius: 8px;
      padding: 1.25rem;
    }
    button {
      border: 0;
      border-radius: 6px;
      padding: 0.7rem 0.9rem;
      background: #693668;
      color: #1b1b3a;
      font-weight: 700;
    }
  </style>
</head>
<body>
  <main>
    <h1>Nudge Tiny Budget Board</h1>
    <p>A retro terminal tiny budget board with progress rings, keyboard shortcuts, and localStorage persistence.</p>
    <button id="spark">Make today count</button>
    <p id="out"></p>
  </main>
  <script>
    const notes = ["Start tiny.", "Ship the useful bit.", "Make it pleasant."];
    spark.onclick = () => out.textContent = notes[Math.floor(Math.random() * notes.length)];
  </script>
</body>
</html>
```

Build note: keep the first version under one screen and make the primary action obvious.
